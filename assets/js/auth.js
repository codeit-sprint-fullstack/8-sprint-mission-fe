document.addEventListener('DOMContentLoaded', () => { // dom이 로드되면 실행
  ...
  const passwordToggleButton = document.querySelector('.input-password-toggle'); // 비밀번호 보기 버튼
  ...
  passwordToggleButton.addEventListener('click', () => {
    ...
    passwordToggleButton.querySelector('img').src = iconSrc;
  });
  ...
  const validateInput = (inputElement, type) => {
    ...
    const showError = (message) => {
      ...
      if (!errorMessage) { // 에러메시지가 존재하지 않을 경우 생성
    ...
    if (inputElement.value === '') { // 값이 없을 경우
      showError(type === 'email' ? '이메일을 입력해주세요.' : type === 'nickname' ? '닉네임을 입력해주세요.' : '비밀번호를 입력해주세요.');
    } else if (type === 'email' && !emailRegex.test(inputElement.value)) { // 이메일 형식에 맞지 않을 경우
      showError('잘못된 이메일 형식입니다.');
    } else if (type === 'password' && inputElement.value.length < 8) { // 비밀번호가 8자 미만일 경우
      showError('비밀번호를 8자 이상 입력해주세요.');
    } else if (type === 'password-check' && inputElement.value !== passwordInput.value) { // 비밀번호 확인 입력 필드가 비밀번호와 다를 경우
      showError('비밀번호가 일치하지 않습니다.');
    } else { // 값이 있고 형식에 맞을 경우
