      ...
      <section className="mb-12">
        {commentLoading && comments.length === 0 ? (
          ...
          <ul className="space-y-4">
            {comments.map((c) => (
              <li key={c.id} className="flex flex-col gap-5 p-4 rounded-lg border" style={{ borderColor: 'var(--gray-100)', backgroundColor: '#fcfcfc' }}>
                <div className="flex justify-between relative">
                  {editingId === c.id ? (
                    <div className="flex gap-2 items-start w-full">
                      <textarea
                        className="flex-1 min-h-20 p-2 rounded border text-sm"
                        style={{ borderColor: 'var(--gray-200)', backgroundColor: '#FFFFFF', color: 'var(--gray-800)' }}
                        value={editingText}
                  ...
                  {editingId !== c.id && (
                    <div className="relative">
                      <button
                        ...
                        onClick={() => setCommentMenuOpenId((prev) => (prev === c.id ? null : c.id))}
                ...
                <div className="flex items-center gap-5">
                  <Image src={c.avatar || "/images/icon/ic_profile.svg"} alt="작성자" width={40} height={40} />
