...
export default function ProductList({ items, isCommon }: ProductListProps) {
  ...
  return (
    ...
    {isCommon ? (
        //일반 상품
        <ul className={`grid gap-y-[40px] gap-x-[24px] list-none p-0 w-full h-fit mx-auto ${gridStyleCommon[devideType]}`}>
        {items.map((item, idx) => {
          return (
      ...
      //베스트 상품
      <ul className={`grid gap-y-[40px] gap-x-[24px] list-none p-0 w-full h-fit mx-auto mb-[24px] ${gridStyleBest[devideType]}`}>
        {items.map((item, idx) => {
          return (
    ...
    <div className="w-full flex flex-col items-start gap-[16px]">
      ...
      <div className="w-full aspect-square overflow-hidden rounded-[16px]">
        {item.images?.length > 0 ? (
          <img src={`${url}/uploads/${item.images[0]}`} alt="productImage" className="w-full h-full object-cover"/>
        ): (
      ...
      <div className="gap-[6px]">
        ...
        <p className="text-[var(--Secondary-800)] text-[16px] font-[700]">{item.price.toLocaleString() + '원'}</p>
