...
function useInputValidation(values){
    ...
    const checkValidation = (name, value=values[name]) => {
        ...
        switch(name){
            case 'name':
                if(value.length===0){msg = '상품명을 입력해주세요'}
                else if(value.length>10){msg = '10자 이내로 입력해주세요'}
                break;
            case 'description':
                if(value.length===0){msg = '상품 소개를 입력해주세요'}
                else if(value.length<10){msg = '10자 이상 입력해주세요'}
                break;
            case 'price':
                if(value.length===0){msg = '판매 가격을 입력해주세요'}
                else if(value.match(/\D+/g)){msg = '숫자로 입력해주세요'}
                break;
            case 'tag':
                if(value.length>5){msg = '5글자 이내로 입력해주세요'}
                break;
            case 'tags':
                break;
        }
